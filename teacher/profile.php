        ...
        <div class="card">
            ...
            <div class="card-body">
                <div style="display:flex;align-items:center;gap:16px;margin-bottom:20px">
                    <div class="avatar avatar-lg" id="prof-avatar" style="width:64px;height:64px;font-size:24px;border-radius:50%"></div>
                    <div>
                        <div style="font-weight:700;font-size:18px" id="prof-name"></div>
                        <div style="color:var(--c-muted);font-size:13px" id="prof-role"></div>
                    </div>
                </div>
                <div class="form-group">
                    <label class="form-label">Кафедра</label>
                    <div id="prof-dept" class="form-static">—</div>
                </div>
                <div class="form-group">
                    <label class="form-label">Должность</label>
                    <div id="prof-position" class="form-static">—</div>
                </div>
                <div class="form-group">
...
(function () {
    ...
    async function loadProfile() {
        try {
            ...
            var s = sess.data[0];
            userId = pick(s,'Пользователь_ID','user_id') || 0;
            var teacherId = pick(s,'Преподаватель_ID','teacher_id') || 0;
            ...
            var role = pick(d,'Роль','role') || 'Преподаватель';
            var dept = pick(d,'Кафедра','Подразделение') || '—';
            var pos  = pick(d,'Должность','position') || '—';
            var email= pick(d,'Email','email') || '';
            var phone= pick(d,'Телефон','phone') || '';

            var ini = fio.split(' ').map(function(p){return p?p[0].toUpperCase():''}).join('').slice(0,2) || 'П';
            ...
            document.getElementById('prof-dept').textContent   = dept;
            document.getElementById('prof-position').textContent = pos;
    ...
    document.getElementById('save-btn').addEventListener('click', async function() {
        ...
        try {
            ...
            if (r && r.success) {
                okEl.textContent = '✅ Данные сохранены.'; okEl.style.display = 'flex';
            } else {
                errEl.textContent = '⚠️ ' + (r && r.message ? r.message : 'Ошибка сохранения.');
                errEl.style.display = 'flex';
            }
        } catch(e) {
            errEl.textContent = '⚠️ Ошибка соединения.'; errEl.style.display = 'flex';
        } finally {
    ...
    document.getElementById('pw-btn').addEventListener('click', async function() {
        ...
        if (newPw.length < 6) { errEl.textContent = '⚠️ Новый пароль должен быть не менее 6 символов.'; errEl.style.display = 'flex'; return; }
        ...
        try {
            ...
            if (r && r.success) {
                ...
                errEl.textContent = '⚠️ ' + (r && r.message ? r.message : 'Ошибка смены пароля.');
                errEl.style.display = 'flex';
            }
        } catch(e) {
            errEl.textContent = '⚠️ Ошибка соединения.'; errEl.style.display = 'flex';
