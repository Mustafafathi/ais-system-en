...
<div class="page-hdr">
    <div>
        ...
        <div class="page-sub">Заявки студентов вашей группы</div>
    </div>
...
(function () {
    ...
    async function load() {
        ...
        error.className       = 'alert alert-err';
        ...
        try {
            ...
            rows.forEach(function(row) {
                ...
                var comment  = pick(row,'Комментарий','Комментарий_Модератора','Комментарий_Куратора') || '—';
    ...
    async function submitDecision(decision) {
        ...
        try {
            ...
            if (r && r.success) {
                var messageRow = Array.isArray(r.data) ? r.data[0] : null;
                closeModal();
                await load();
                var info = document.getElementById('result');
                info.textContent = pick(messageRow, 'Сообщение') || 'Решение сохранено, посещаемость и уведомление обновлены.';
